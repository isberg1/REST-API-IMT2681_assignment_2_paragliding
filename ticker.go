...
func apiTicker(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	latestimeStamp, ok := MgoDB.GetLatest()
	if !ok {
		http.NotFound(w, r)
		return
	}
	var nextTimeStamp int64
	nextTimeStamp, ok = MgoDB.GetOldest()
	if !ok {
		http.Error(w, "", http.StatusNoContent)
		return
	}
	...
	firsID, ok := MgoDB.GetByTimstamp(nextTimeStamp)
	if !ok {
		http.Error(w, "", http.StatusNoContent)
		return
	}
	...
	for i := 1; i < nr; i++ {
		temp, err1 := MgoDB.GetBiggerThen(nextTimeStamp)
		if err1 != nil {
...
func apiTimestamp(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if MgoDB.Count() == 0 || MgoDB.Count() == 1 {
		/* alternative sulution:
		ticker := Ticker{
			TLatest: -1,
			TStart:-1,
	...
	var idArray = make([]ResponsID, 0, 0)
	firsID, err1 := MgoDB.GetBiggerThen(nextTimeStamp)
	if err1 != nil {
